...
module E04_ObjImporter
  ...
  def self.do_import(file_path, options)
    puts "[e04 OBJ Importer] Importing: #{file_path}"
    puts "[e04 OBJ Importer] Options: up_axis=#{options['up_axis']}, unit=#{options['source_unit']}"

    begin
      # 1. 解析 OBJ 檔案
      Sketchup.status_text = 'Parsing OBJ file...'
      obj_data = ObjParser.parse(file_path)
      puts "[e04 OBJ Importer] Vertices: #{obj_data[:vertices].length}, " \
      ...
      if obj_data[:mtl_file]
        ...
        if mtl_path && File.exist?(mtl_path)
          ...
          puts "[e04 OBJ Importer] Materials: #{materials_data.length}"
        else
          puts "[e04 OBJ Importer] WARNING: MTL file not found: #{obj_data[:mtl_file]}"
        end
      end
  ...
  unless file_loaded?(__FILE__)
    ...
    menu.add_item('e04 OBJ Importer') { import_obj_file }
